    ...
    stages {
        ...
        stage('Deploy to Production environment') {
            ...
            steps {
                ...
                milestone(1)
                dockerStop = 'docker stop php-app'
                dockerRemove = 'docker rm php-app'
                dockerRun = "docker run --restart always --name php-app -p 80:80 -d krishh11234/php-app:${env.BUILD_NUMBER}"
                sshagent(['prod-creds']) {
                    script {
                        try {
